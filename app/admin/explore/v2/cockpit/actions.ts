...
export async function purgeTestSubscribers(): Promise<ActionResult> {
  try {
    ...
    const { data, error } = await db
      ...
      .delete()
      .or("email.ilike.%+test%,email.ilike.%test+%,email.ilike.test@%,email.ilike.%@test.%,email.ilike.%@example.%")
      .select("id");
...
export async function revalidateAllTickerPages(): Promise<ActionResult> {
  try {
    ...
    return {
      ok: true,
      message: `Revalidated ${tickers.length} ticker page${tickers.length !== 1 ? "s" : ""} + index`,
    };
  } catch (err) {
    return { ok: false, error: String(err) };
  }
